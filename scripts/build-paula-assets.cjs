...
// Usage: node scripts/build-paula-assets.cjs SOURCE_SHEET
...
(async()=>{
 ...
 const xs=Array(4).fill([0,313,627,940,1254]),ys=[0,314,627,938,1254],frames=[];
 for(let pose=0;pose<16;pose++){
  ...
  const meta=await sharp(crop).metadata(),scale=Math.min(234/288,258/meta.width,254/meta.height);
  ...
  if(pose===0)await sharp(crop).webp({quality:95,alphaQuality:100}).toFile('assets/paula-portrait-v1.webp');
 }
 await sharp({create:{width:1080,height:1080,channels:4,background:'#00000000'}}).composite(frames).webp({quality:94,alphaQuality:100}).toFile('assets/paula-atlas-v1.webp');
 console.log('Saved Paula atlas and clean selection portrait.');
})();
