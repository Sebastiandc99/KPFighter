...
// Usage: node scripts/build-padrino-assets.cjs SOURCE_SHEET
...
(async()=>{
 ...
 const xs=[[0,315,638,975,1254],[0,330,640,970,1254],[0,315,640,970,1254],[0,295,637,975,1254]],ys=[0,310,614,908,1254],frames=[];
 for(let pose=0;pose<16;pose++){
  const row=Math.floor(pose/4),col=pose%4,left=xs[row][col],top=ys[row],width=xs[row][col+1]-left,height=ys[row+1]-top;
  ...
  const meta=await sharp(crop).metadata(),scale=Math.min(234/281,258/meta.width,254/meta.height);
