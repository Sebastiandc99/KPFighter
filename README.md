...
### Online · 20260908b
**JUGAR ONLINE → CREAR SALA** genera un código de seis caracteres. El rival entra por **UNIRSE A SALA**. Cada uno elige y confirma su personaje en su dispositivo; cuando ambos están listos comienza la pelea en el escenario activo del creador. Cada dispositivo usa los controles habituales del jugador 1 (WASD, J/K/L, I, H, O) o los controles táctiles. Los luchadores conservan su posición 1P/2P en ambas pantallas.

El host ejecuta el mismo motor, incluidos IA desactivada para el rival, todos los poderes, daño, rondas y puntaje. El invitado envía controles con números de secuencia y confirmación para evitar golpes duplicados. Recibe estados completos e interpola posiciones; no decide impactos. WebRTC transmite a unos 30 estados/s cuando la red lo permite. Supabase Realtime privado sirve para señalización y como alternativa a unos 12 estados/s. La latencia depende de la conexión; esta primera versión no implementa rollback. El ping muestra el recorrido de ida y vuelta. Una interrupción prolongada muestra una salida al menú; la pausa se comparte. Las salas duran hasta 45 minutos y admiten dos participantes. Se necesita mantener abierto el juego en el dispositivo host.

GitHub Pages sigue siendo el único hosting del juego. `online-config.js` contiene únicamente la URL y clave **anon pública**. `supabase/functions/kp-rooms/index.ts` usa las credenciales administrativas solo desde el entorno del servidor: crea identidades temporales sin datos personales ni correos enviados, emite una sesión y asigna la sala. El arranque exige la clave pública y limita intentos; las acciones de cierre validan el JWT. Por eso la función valida la autenticación internamente (`verify_jwt=false`). No poner claves administrativas en archivos del cliente.

Las migraciones versionan las políticas: nadie puede listar salas públicamente ni modificar sus plazas desde el navegador. Cada participante lee solamente su sala y los canales privados correspondientes. Solo el creador puede escribir al canal del host; el invitado tiene su propio canal. La asignación de la segunda plaza es atómica. Se limpian salas vencidas e identidades de juego temporales; no se borran usuarios normales. El ranking previo se conserva y los resultados online usan su categoría de versus, con formulario en el dispositivo ganador.

Pruebas: `node --test tests/combat.test.cjs tests/online.test.cjs`. Referencias del transporte: https://supabase.com/docs/guides/realtime/protocol y https://supabase.com/docs/guides/realtime/authorization.
